...
class IcmsTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->setTable('icms');
        $this->setDisplayField('id_icms');
        $this->setPrimaryKey('id_icms');
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ->integer('id_icms')
            ->allowEmpty('id_icms', 'create');

        $validator
            ->scalar('codigo_icms')
            ->maxLength('codigo_icms', 16)
            ->requirePresence('codigo_icms', 'create')
            ->notEmpty('codigo_icms');

        $validator
            ->scalar('desc_icms')
            ->requirePresence('desc_icms', 'create')
            ->notEmpty('desc_icms');
